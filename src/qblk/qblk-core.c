...
int qblk_blk_erase_sync(struct qblk *qblk, struct ppa_addr ppa)
{
	...
	qblk_calculate_write_amplification(qblk, 1, 0, 0, 0, 0, 1);
						...
						u64 offset_in_line, int dir)
{
	...
	//pr_notice("%s, offsetInLine=%llu, ppa=0x%llx\n",
	//				__func__, offset_in_line, ppa.ppa);
	...
	if (meta->smeta_sec == 1) {
		...
		if (dir == QBLK_WRITE) {
			...
			meta_list[0].lba = lba_list[offset_in_line] = addr_empty;
		}
		goto submit_rqd;
	}
...
submit_rqd:
	...
	qblk_calculate_write_amplification(qblk, dir == QBLK_WRITE, meta->smeta_sec, 0, 0, 0, 0);
	ret = qblk_submit_io_sync(qblk, &rqd);
	if (ret) {
				...
				struct ch_info *chi, struct qblk_line *line)
{
	...
	if (!qblk_line_init_bb(qblk, chi, line, 0)) {
		ret = -EINTR;
		goto fail;
	}
				...
				struct qblk_line *cur)
{
	...
	smeta_buf->window_wr_lun = cpu_to_le32(geo->num_lun);
...
void qblk_line_close(struct qblk *qblk, struct qblk_line *line)
{
	...
	//if (!chi->ch_index)
	//	pr_notice("%s, line(%u) moves to gc list (%u)\n",
	//			__func__, line->id, line->gc_group);
				...
				struct ch_info *chi, struct qblk_line *line)
{
	...
	//pr_notice("%s,close meta line of ch[%d],line[%u], nr_valid_lbas=%u\n",
	//			__func__, chi->ch_index, line->id, line->nr_valid_lbas);
		...
		struct ch_info *chi, unsigned long *lun_bitmap)
{
	...
	int num_lun = geo->num_lun;
	int ret;

	while ((lun = find_next_bit(lun_bitmap, num_lun, lun + 1)) < num_lun) {
...
static void qblk_invalidate_range(struct qblk *qblk, sector_t slba,
				  unsigned int nr_secs)
{
	sector_t lba;
	struct ppa_addr ppa_empty;

	qblk_ppa_set_empty(&ppa_empty);

	for (lba = slba; lba < slba + nr_secs; lba++) {
		struct ppa_addr ppa;

		ppa = qblk_trans_map_atomic_get_and_set(qblk, lba, ppa_empty);

		if (!qblk_addr_in_cache(ppa) && !qblk_ppa_empty(ppa))
			qblk_map_invalidate(qblk, ppa);
	}
}
...
void qblk_discard_req(struct qblk *qblk, struct request *req)
{
	...
	__rq_for_each_bio(bio, req) {
		...
		//pr_notice("%s-slba=%lu nr_secs=%lu\n",
		//			__func__, slba, nr_secs);
		qblk_invalidate_range(qblk, slba, nr_secs);
	}
			 ...
			 void *emeta_buf)
{
	...
	ppa_list = meta_list + qblk_dma_meta_size(qblk);
...
next_rq:
	...
	for (i = 0; i < rqd.nr_ppas; ) {
		...
		if (rq_ppas == 1)
			rqd.ppa_addr = offset_in_line_to_gen_ppa(qblk, paddr, chi->ch_index, id);
		else
...
void qblk_write_should_kick(struct qblk *qblk, int index, int force_kick)
{
	...
	if (secs_avail >= qblk->min_write_pgs)
		...
		mod_timer(&qblk->wtimers[index].timer, jiffies + msecs_to_jiffies(QBLK_FUA_MERGE_WINDOW_SIZE));
}
#else
void qblk_write_should_kick(struct qblk *qblk, int index, int force_kick)
{
