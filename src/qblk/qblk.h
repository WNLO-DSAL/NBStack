...
/* Uncomment to monitor write amplification */
//#define MONITOR_WRITE_AMPLIFICATION

#define QBLK_FUA_MERGE_WINDOW_SIZE (2)
...
/* Uncomment to print qblk's writeback size */
//#define PRINT_QBLK_WB_SIZE
...
struct qblk_rb {
	...
	unsigned int sync;		/* Synced - backpointer that signals
					 ...
					 */
	atomic_t flush_point;/* Sync point - last entry that must be
				  * flushed to the media. Used with
				  * REQ_FUA.
				  * Draining thread can writeback entries
				  * after the flush_point together with entries
				  * before the flush_point.
				  */
...
struct ch_info {
	...
	int nr_reserved; /* Number of lines reserved for no-FTL IO */

	struct qblk_line **reserved_lines;
...
struct qblk_gc_rq {
	...
	struct qblk_sec_meta meta[QBLK_MAX_REQ_ADDRS];
	int nr_secs;
	int secs_to_gc;
	struct list_head list;
};
...
struct qblk {
	...
	struct gendisk *disk;
	unsigned long reserve_start; /* reserved raw IO start ADDR */
	...
	struct qblk_addr_format reserve_ppaf;
	...
	atomic_t total_submitted;
	atomic_t total_submitted2;
	atomic_t total_finished;
	atomic_t total_finished2;
	...
	unsigned long long rb_req_count[32];
	int rb_need_count;
#endif

#ifdef MONITOR_WRITE_AMPLIFICATION
	atomic_t nsm, nem, nv, np, ner;
#endif
...
struct qblk_rs {
	...
	struct qblk_reserve_space_context *context;
	spinlock_t bitmap_lock;
	unsigned long *channel_bitmap;
	struct qblk_perch_rs *per_ch_rs;
	atomic_t status;
};

static inline int qblk_dma_meta_size(struct qblk *qblk)
{
	return max_t(int, sizeof(struct qblk_sec_meta), qblk->oob_meta_size)
	       * NVM_MAX_VLBA;
}
...
static inline struct ppa_addr gen_lba_to_reserved_ppa(struct qblk *qblk, u64 lba)
{
	unsigned long long lpn = lba - qblk->reserve_start;
	struct qblk_addr_format *reserve_ppaf = &qblk->reserve_ppaf;
	struct ppa_addr ppa;
	int chnl;
	struct ch_info *chi;

	ppa.ppa = 0;
	ppa.g.pg = lpn & reserve_ppaf->pg_mask;
	chnl = ppa.g.ch = lpn & reserve_ppaf->ch_mask;
	ppa.g.lun = lpn & reserve_ppaf->lun_mask;
	chi = &qblk->ch[chnl];
	ppa.g.blk = 
		chi->reserved_lines[lpn & reserve_ppaf->blk_mask]->id;
	pr_notice("%s, lpn=0x%llx, pgmask=0x%llx\n",
				__func__, lpn, reserve_ppaf->pg_mask);
	return ppa;
}
...
static inline int qblk_check_io(struct qblk *qblk, struct nvm_rq *rqd)
{
	...
	if (rqd->opcode == NVM_OP_PWRITE) {
		...
		for (i = 0; i < rqd->nr_ppas; i++) {
			line = qblk_ppa_to_structline(qblk, ppa_list[i]);

			spin_lock(&line->lock);
			if (line->state != QBLK_LINESTATE_OPEN) {
				pr_err("bad ppa: line:%d,state:%d\n",
							line->id, line->state);
				WARN_ON(1);
...
static inline int qblk_has_reserved(struct qblk *qblk)
{
	return qblk->reserve_start != ADDR_EMPTY;
}
...
{
	return lun_in_ch*geo->num_ch + ch_idx;
}
...
extern unsigned int qblk_rb_read_count_for_drain(struct qblk_rb *rb);
extern unsigned int qblk_rb_read_commit(struct qblk_rb *rb,
				unsigned int nr_entries);
extern inline struct qblk_rb_entry *qblk_rb_entry_by_pos(struct qblk_rb *rb, unsigned int pos);
extern struct qblk_w_ctx *qblk_rb_w_ctx(struct qblk_rb *rb, unsigned int pos);
extern unsigned int qblk_rb_flush_point_count(struct qblk_rb *rb, unsigned int flush_point);
extern void qblk_rb_sync_all_l2p(struct qblk *qblk);
extern int qblk_rb_may_write_user(struct qblk *qblk,
				unsigned int rbid,
...
extern void qblk_persist_work_release(struct qblk_persist_work *persist_work);
extern blk_status_t qblk_rq_write_to_cache(struct qblk *qblk,
				struct qblk_queue *pq,
				struct request *req,
				unsigned long flags);
extern blk_status_t qblk_flush_req(struct request_queue *q,
...
extern int qblk_rl_user_maynot_insert(struct qblk *qblk, int nr_entries);
...
extern void qblk_rl_init(struct qblk_rl *rl, int budget);
...
int qblk_line_is_full(struct qblk_line *line);
...
int qblk_line_recov_alloc(struct qblk *qblk,
				struct ch_info *chi, struct qblk_line *line);
void qblk_line_recov_close(struct qblk *qblk,
			struct qblk_line *line);
...
extern void qblk_update_map(struct qblk *qblk, sector_t lba,
			struct ppa_addr ppa);
int qblk_update_map_test(struct qblk *qblk, sector_t lba, struct ppa_addr ppa);
...
extern void qblk_write_should_kick(struct qblk *qblk, int index, int force_kick);
extern void qblk_write_force_kick_all(struct qblk *qblk);
...
/* qblk-write.c */
extern void qblk_end_persist_point(struct qblk_rb *rb,
							struct qblk *qblk,
...
extern void qblk_debug_time(struct qblk *qblk, int *pindex,
				int chnl, struct qblk_debug_entry entry);
extern void qblk_debug_add_endreq(struct request_queue *q,
									int rw,
...
extern void inline qblk_printTimeMonotonic(const char *ch, int line);

#ifdef MONITOR_WRITE_AMPLIFICATION

void qblk_debug_calc_wa(struct qblk *qblk, int en, int nsm, int nem, int nv, int np, int ner);

#define qblk_calculate_write_amplification(qblk, en, nsm, nem, nv, np, ner) \
					qblk_debug_calc_wa(qblk, en, nsm, nem, nv, np, ner)
#else

#define qblk_calculate_write_amplification(a,b,c,d,e,f,g)
...
extern int qblk_gc_init_step1(struct qblk *qblk);
extern int qblk_gc_init(struct qblk *qblk);
extern void qblk_gc_exit_for_step1(struct qblk *qblk);
