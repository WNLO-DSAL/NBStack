			...
			unsigned int valid_secs, unsigned int rb_count)
{
	...
	if (qblk_has_reserved(qblk) && nr_secs == 1) {
		/* We need to take no-FTL IO into account */
		entry = qblk_rb_entry_by_pos(&qblk->mqrwb[rb_count], sentry);
		w_ctx = qblk_rb_w_ctx(&qblk->mqrwb[rb_count], sentry);
		if (w_ctx->lba != ADDR_EMPTY && w_ctx->lba >= qblk->reserve_start) {
			/* A line contains no smeta and emeta when it's a no-FTL line. */
			w_ctx->ppa = *ppa_list = gen_lba_to_reserved_ppa(qblk, w_ctx->lba);
			pr_notice("%s, noFTL IO, lba=0x%llxppa=0x%llx\n",
					__func__, w_ctx->lba, w_ctx->ppa.ppa);
			pr_notice("ppa: :ch:%d,lun:%d,blk:%d,pg:%d,pl:%d,sec:%d\n",
				w_ctx->ppa.g.ch, w_ctx->ppa.g.lun, w_ctx->ppa.g.blk,
				w_ctx->ppa.g.pg, w_ctx->ppa.g.pl, w_ctx->ppa.g.sec);
			meta_list->lba = cpu_to_le64(w_ctx->lba);
			meta_list->reserved = entry->metadata;
			qblk_mark_rq_luns(qblk, ppa_list, nr_secs, lun_bitmap);
			return;
		}
	}

	/*if (rb_count >= qblk->nr_queues) {
		pr_notice("%s, rb_count(%u) > nrq(%u)\n",
					__func__, rb_count, qblk->nr_queues);
	}

	if (nr_secs == 1) {
		w_ctx = qblk_rb_w_ctx(&qblk->mqrwb[rb_count], sentry);
		if (w_ctx->lba >= qblk->reserve_start) {
			pr_notice("%s, lba(%llu) >= reserve_start(%lu)\n",
					__func__, w_ctx->lba, qblk->reserve_start);
		}
	}*/
			...
			unsigned int off, unsigned int rb_count)
{
	...
	if (rqd->nr_ppas == 1) {
		map_secs = (off + min > valid_secs) ? (valid_secs % min) : min;
		qblk_map_page_data(qblk, chi, sentry + off, &rqd->ppa_addr,
					lun_bitmap, &meta_list[off], map_secs, rb_count);
		return;
	}
		    ...
		    unsigned int rb_count)
{
	...
	for (i = 0; i < rqd->nr_ppas; i += min) {
		map_secs = (i + min > valid_secs) ? (valid_secs % min) : min;
		if (rqd->nr_ppas == 1) {
			qblk_map_page_data(qblk, chi, sentry + i, &rqd->ppa_addr,
					lun_bitmap, &meta_list[i], map_secs, rb_count);
			erase_lun = qblk_ppa_to_posinsidechnl(geo, rqd->ppa_addr);
		} else {
			qblk_map_page_data(qblk, chi, sentry + i, &rqd->ppa_list[i],
					lun_bitmap, &meta_list[i], map_secs, rb_count);
