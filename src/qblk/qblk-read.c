...
static void qblk_end_io_read(struct nvm_rq *rqd)
{
	...
	int need_copy;
	unsigned int bvlen;
	unsigned long *metalist = rqd->meta_list;
	unsigned long **meta_array;
	...
	/*
	 * This downside bio is created by us.
	 * Thus, it's save to re-iterate the whole bio.
	 */
	downside_bio->bi_iter.bi_bvec_done = 0;
	downside_bio->bi_iter.bi_idx = 0;
	downside_bio->bi_iter.bi_size = rqd->nr_ppas * QBLK_EXPOSED_PAGE_SIZE;
	i = 0;

	meta_array = downside_bio->bi_meta.bi_metabase;
	need_copy = meta_array?1:0;
	bio_for_each_bvec(bvl, downside_bio, downside_bio->bi_iter) {
		bvlen = bvl.bv_len;
		while (bvlen)
		{
			if (need_copy && *meta_array)
				**meta_array = metalist[i];
					...
					struct qblk *qblk, struct request *req)
{
	...
	if (blba + nr_secs > qblk->rl.nr_secs || nr_secs > QBLK_MAX_REQ_ADDRS) {
		WARN(1, "qblk: read lba out of bounds[%llu] (lba:%llu, nr:%d)\n",
					qblk->rl.nr_secs,
					(unsigned long long)blba, nr_secs);
		return BLK_STS_IOERR;
	}

	/*if (nr_secs > QBLK_MAX_REQ_ADDRS) {
		newbio = bio_split(bio,
			QBLK_MAX_REQ_ADDRS << 3,
	...
	downside_bio = bio_alloc_withmeta(GFP_ATOMIC | __GFP_ZERO, nr_secs, nr_secs);
	if (!downside_bio)
		goto errout1;
	...
	ppalist = (struct ppa_addr *)(metalist + qblk_dma_meta_size(qblk));
	//pr_notice("%s, blba=%ld, nr_secs=%u, nrseg=%d\n",
	//	__func__, blba, nr_secs, blk_rq_nr_phys_segments(req));
	__rq_for_each_bio(iter.bio, req) {
		pmeta = (iter.bio)->bi_meta.bi_metabase;
		bio_for_each_bvec(bvec, iter.bio, iter.iter) {
...
retry:
				...
				if (qblk_ppa_empty(ppa)) {
					...
					kref_get(&line->ref);
					if (pmeta && !bio_add_page_with_meta(downside_bio, bvec.bv_page,
								QBLK_EXPOSED_PAGE_SIZE, bvoffset,
								*pmeta)) {
						rv = BLK_STS_IOERR;
						goto errout3;
					} else if (!bio_add_page(downside_bio, bvec.bv_page,
								QBLK_EXPOSED_PAGE_SIZE, bvoffset)) {
						rv = BLK_STS_IOERR;
						goto errout3;
					}
...
static void qblk_fill_meta_to_gc_rq(struct qblk *qblk,
						struct qblk_gc_rq *gc_rq,
						void *rqd_meta)
{
	int nrsec = gc_rq->nr_secs;
	int i;
	u64 *lba_list = gc_rq->lba_list;
	struct qblk_sec_meta *meta = gc_rq->meta;
	struct qblk_sec_meta *meta_read = rqd_meta;
...
int qblk_submit_read_gc(struct qblk_gc *gc, struct qblk_gc_rq *gc_rq)
{
	...
	qblk_fill_meta_to_gc_rq(qblk, gc_rq, rqd.meta_list);
